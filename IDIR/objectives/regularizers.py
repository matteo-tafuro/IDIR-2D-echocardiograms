...
def compute_hyper_elastic_loss(
    ...
    cofactors = torch.zeros(batch_size, 1)

    # Compute elements of cofactor matrices one by one
    cofactors[:, 0] = torch.det(grad_y)

    # Compute area loss
    area_loss = torch.pow(cofactors, 2) - 1
    area_loss = torch.abs(area_loss)
